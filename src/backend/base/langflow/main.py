...
from langflow.initial_setup.setup_operations import seed_operations_data
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        try:
            ...
            # Gate: Seed operations module data (tenants, action types)
            try:
                async with session_scope() as session:
                    await seed_operations_data(session)
            except Exception as e:
                await logger.awarning(f"Failed to seed operations data: {e}")
